...
def register():
    try :
        ...
        if sl == 0:
            ...
            print(currentDate)
            sql = "insert into user (fullname,username,email,phonenumber,isActive,password,createdAt,updatedAt) value ('"+str(fullname)+"','"+str(user)+"','"+str(email)+"','"+str(phonenumber)+"','1','"+str(hashedPassword.decode())+"',NOW(),NOW())"
            cursor = db.cursor()
            cursor.execute(sql)
            ...
            mailuser='notification@leeon.vn'
            mailpassword='Leeon@123'
            mailserver='smtp.office365.com'
            subject="Thông báo tài khoản CRM-LEEON"
            msg = MIMEMultipart()
            msg['From'] = 'notification@leeon.vn'
            msg['To'] = str(email)
...
@login.route('/dangnhap', methods=['POST'])
def dangnhap():
    try :
...
@login.route('/list_group', methods=['GET'])
def list_group():
    try :
        db = pymysql.connect(host="172.17.0.1",    # your host, usually localhost
                     user="leeoncrm",         # your username
...
@login.route('/reset_password', methods=['POST'])
def reset_password():
    ...
    try :
        ...
        mailpassword='Leeon@123'
        mailserver='smtp.office365.com'
        subject="Đổi mật khẩu CRM-LEEON"
        msg = MIMEMultipart()
        msg['From'] = 'notification@leeon.vn'
        msg['To'] = str(email)
        msg['Subject'] = "%s" % Header(subject, 'utf-8')
...
@login.route('/register_v2', methods=['POST'])
def register_v2():
    try :
        db = pymysql.connect(host="172.17.0.1",    # your host, usually localhost
                     user="leeoncrm",         # your username
        ...
        if sl == 0:
            ...
            currentDate = time.strftime("%Y-%m-%d %H:%M:%S")
            print(currentDate)
            sql = "insert into user (partner1,partner2,partner3,fullname,username,email,phonenumber,isActive,password,createdAt,updatedAt,partner_code) value ('"+str(nickname1)+"','"+str(nickname2)+"','"+str(nickname3)+"','"+str(fullname)+"','"+str(user)+"','"+str(email)+"','"+str(phonenumber)+"','1','"+str(hashedPassword.decode())+"',NOW(),NOW(),'"+str(partner_code)+"')"
            cursor = db.cursor()
            cursor.execute(sql)
            try:
                insertgroup ="insert into leeon_crm.user_group(userId,groupId) value ((select id from user where username = '"+str(user)+"'),(select id from leeon_crm.group where name = '"+str(group)+"'))"
